...
import { containsValue, findByte, messageWithOffset, parseString, textDecoder } from "./utils.js";
...
export function parseSectionHeader(buffer: Uint8Array, 
    startOffset: number, expectedType: SectionTypeMarker): ParseResult<SectionHeader> {
        ...
        if (buffer[offset] !== 0x00) {
            throw new Error(messageWithOffset(`Expected section size marker 0x00, found 0x${buffer[offset].toString(16)}`, offset));
        }
        offset += 1;
