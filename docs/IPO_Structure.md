...
## 13. Implicit Compiler Additions

The INPA compiler performs several implicit actions when generating IPO files:

- **Reserved Global Variables:** The compiler reserves approximately 14 global variables (indices 0-13) for internal system use. User-defined global variables start from index 14 onwards.
- **Auto-generated Functions:** The functions `__inpa_startup__` and `__inpa_shutdown__` are always generated by the compiler to handle initialization and cleanup logic.
- **UI Handles:** The special handle `0xf00` is used by the startup/shutdown logic for internal UI management.
